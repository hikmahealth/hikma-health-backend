...
def upgrade():
    op.create_table(
        ...
        sa.Column('prescribed_at', sa.DateTime(timezone=True), nullable=False,
                  default=datetime.now(UTC), server_default=sa.text('CURRENT_TIMESTAMP')),
        ...
        sa.Column('deleted_at', sa.DateTime(timezone=True),
                  nullable=True, default=None, server_default=None),
        sa.Column('last_modified', sa.DateTime(timezone=True), nullable=False, default=datetime.now(
            UTC), server_default=sa.text('CURRENT_TIMESTAMP'), onupdate=sa.text('CURRENT_TIMESTAMP')),
    ...
    # Create foreign keys
    op.create_foreign_key('fk_prescriptions_patient',
                          'prescriptions', 'patients', ['patient_id'], ['id'])
    op.create_foreign_key('fk_prescriptions_provider',
                          'prescriptions', 'users', ['provider_id'], ['id'])
    op.create_foreign_key('fk_prescriptions_pickup_clinic',
                          'prescriptions', 'clinics', ['pickup_clinic_id'], ['id'])

    # Create indexes
    op.create_index('ix_prescriptions_patient_id',
                    'prescriptions', ['patient_id'], unique=False)
    op.create_index('ix_prescriptions_pickup_clinic_id',
                    'prescriptions', ['pickup_clinic_id'], unique=False)


def downgrade():
    # Drop indexes
    op.drop_index('ix_prescriptions_pickup_clinic_id',
                  table_name='prescriptions')
    op.drop_index('ix_prescriptions_patient_id', table_name='prescriptions')

    # Drop foreign keys
    op.drop_constraint('fk_prescriptions_pickup_clinic',
                       'prescriptions', type_='foreignkey')
    op.drop_constraint('fk_prescriptions_patient',
                       'prescriptions', type_='foreignkey')
    op.drop_constraint('fk_prescriptions_provider',
                       'prescriptions', type_='foreignkey')
