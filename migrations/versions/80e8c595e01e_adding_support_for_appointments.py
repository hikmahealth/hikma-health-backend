...
def upgrade():
	op.create_table(
		...
		sa.Column(
			'created_at', sa.TIMESTAMP(True), nullable=False, default=datetime.now(UTC)
		),
		sa.Column(
			'updated_at', sa.TIMESTAMP(True), nullable=False, default=datetime.now(UTC)
		),
		sa.Column(
			'last_modified',
			sa.TIMESTAMP(True),
	...
	op.create_foreign_key(
		'fk_appointment_clinic', 'appointments', 'clinics', ['clinic_id'], ['id']
	)
	op.create_foreign_key(
		'fk_appointment_patient', 'appointments', 'patients', ['patient_id'], ['id']
	)
	op.create_foreign_key(
		'fk_appointment_user', 'appointments', 'users', ['user_id'], ['id']
	)
	op.create_foreign_key(
		'fk_appointment_provider', 'appointments', 'users', ['provider_id'], ['id']
	)
	op.create_foreign_key(
		'fk_appointment_current_visit',
		'appointments',
		'visits',
		['current_visit_id'],
		['id'],
	...
	op.create_foreign_key(
		'fk_appointment_fulfilled_visit',
		'appointments',
		'visits',
		['fulfilled_visit_id'],
		['id'],
